...
export default function HexagonMirror() {
  ...
  const h1Opacity = useTransform(scrollYProgress, [0.0, 0.04, 0.083, 0.133], [0, 1, 1, 0])
  const h1Y       = useTransform(scrollYProgress, [0.0, 0.04, 0.083, 0.133], [30, 0, 0, -30]) // Departs natively UPward

  // ── H2: "in a single assessment."
  // Chains immediately after H1 reaches 0 — guaranteed no visual overlap
  const h2Opacity = useTransform(scrollYProgress, [0.133, 0.207, 0.70, 0.80], [0, 1, 1, 0])
  const h2Y       = useTransform(scrollYProgress, [0.133, 0.207], [30, 0]) // Drifts natively UPward into the center
